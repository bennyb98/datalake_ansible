...
def get_host_groups(inventory, port_type, refresh=False, cloud=None):
    ...
    if is_cache_stale(cache_file, cache_expiration_time, refresh=refresh):
        groups = to_json(get_host_groups_from_cloud(inventory, port_type))
...
def get_host_groups_from_cloud(inventory, port_type):
...
def parse_args():
    ...
    parser.add_argument('--port-type', choices=['fixed', 'floating'],
                        default='fixed', help='Choose type of IP address')
...
def main():
    ...
    try:
        ...
        if args.list:
            output = get_host_groups(inventory, port_type=args.port_type, refresh=args.refresh, cloud=args.cloud)
        elif args.host:
